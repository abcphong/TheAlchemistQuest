...
func initialize_item(item_name: String, item_quantity: int):
	print("[DEBUG-SLOT] Khởi tạo item trong slot ", slot_index, ": ", item_name, " x ", item_quantity)
	#Clear existing item
	if item:
		remove_child(item)
		item.queue_free()
...
func pickFromSlot() -> Control :
	print("[DEBUG-SLOT] Lấy item từ slot ", slot_index)
	if item == null:
		return null
...
func putIntoSlot(new_item: Control) -> bool:
	print("[DEBUG-SLOT] Đặt item vào slot ", slot_index, ": ", new_item.item_name)
	if new_item == null:
		return false
