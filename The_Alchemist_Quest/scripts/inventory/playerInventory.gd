...
func add_item(item_name, item_quantity):
	...
	for slot_index in inventory:
		...
		if slot != null and slot[0] != null and slot[0] == item_name:
	...
	print("Failed to add item: ", item_name, " - No available slots or stack space")
	emit_signal("inventory_updated")
	return false
	
func remove_item(slot: SlotClass,is_hotbar: bool = false):
	if is_hotbar:
		hotbar[slot.slot_index] = [null, 0]
	else:
		inventory[slot.slot_index] = [null, 0]
	emit_signal("inventory_updated")
