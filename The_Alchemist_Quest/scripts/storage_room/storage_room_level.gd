...
func _ready():
	print("[StorageRoomLevel] Khởi tạo phòng lưu trữ")
	...
	if game_manager and game_manager.storage_door_entry_position != Vector2.ZERO:
		# Lấy tham chiếu đến người chơi từ BaseLevel
		var player = get_node_or_null("BaseLevel/Player")
		if player:
			player.global_position = game_manager.storage_door_entry_position
			print("[StorageRoomLevel] Đặt người chơi tại vị trí:", game_manager.storage_door_entry_position)
		else:
			print("[StorageRoomLevel] CẢNH BÁO: Không tìm thấy Player node trong BaseLevel")
...
func _on_exit_door_body_entered(body):
	if body.is_in_group("Player"):
		...
		if interaction_prompt:
			...
			print("[StorageRoomLevel] Người chơi đã vào vùng cửa ra")
...
func _on_exit_door_body_exited(body):
	if body.is_in_group("Player"):
		...
		if interaction_prompt:
			interaction_prompt.visible = false
			print("[StorageRoomLevel] Người chơi đã rời vùng cửa ra")

# Xử lý khi người chơi tương tác với cửa ra
func exit_room():
	print("[StorageRoomLevel] Người chơi rời khỏi phòng lưu trữ")
	...
	if game_manager and game_manager.storage_door_entry_position != Vector2.ZERO:
		...
		print("[StorageRoomLevel] Sử dụng vị trí đã lưu để quay về:", return_position)
	else:
		print("[StorageRoomLevel] Không tìm thấy vị trí vào, sử dụng vị trí mặc định:", return_position)
	...
	if game_manager:
		...
		print("[StorageRoomLevel] Đã lưu vị trí quay về:", return_position)
	...
	if save_load_manager:
		...
		print("[StorageRoomLevel] Đã lưu trạng thái tạm thời trước khi chuyển cảnh")
